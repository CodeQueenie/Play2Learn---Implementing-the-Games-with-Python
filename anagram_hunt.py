...
class AnagramHunt(Game):
    ...
    def users_choice(self):
        ...
        self.word_length = 0  # Initialize variable
        acceptable_range = [5, 6, 7, 8]  # Define acceptable range to be between 5 and 8
        # Initial prompt moved outside the loop
        print(f"Please enter a word length {acceptable_range}: ", end="")  # Notice 'end=""' to avoid new line
        
        while self.word_length not in acceptable_range:
            try:
                user_input = input()  # Take input
                self.word_length = int(user_input)  # Convert input to integer
                if self.word_length not in acceptable_range:  # If input is integer but not in range,
                    # inform user and keep the cursor on the same line for the next input.
                    print(f"That is not a correct word length. Please try again {acceptable_range}: ", end="")
            except ValueError:
                # If input is not an integer, inform user and keep the cursor on the same line for the next input.
                print(f"That is not a correct word length. Please try again {acceptable_range}: ", end="")
